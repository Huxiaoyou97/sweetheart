...
function injectCodeExample(source) {
    ...
    return source.replace(codeRE, '<div>$1</div>')
}
...
function markdownToVue(source, options) {
    ...
    return `
<template><div class="xy-site-doc">${templateString}</div></template>
...
function SweetheartMarkdownVitePlugin(options) {
    return {
        name: 's-markdown-vite-plugin',
        enforce: 'pre',
        transform(source, id) {
            if (!/\.md$/.test(id)) {
                return
...
module.exports = SweetheartMarkdownVitePlugin
