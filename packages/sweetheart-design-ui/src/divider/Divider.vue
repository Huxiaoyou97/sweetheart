<template>
  <div
    class="s-divider"
    :class="[
      vertical ? 's-divider--vertical' : null,
      withText ? 's-divider--with-text' : null,
      isInset ? 's-divider--inset' : null,
      dashed ? 's-divider--dashed' : null,
    ]"
    :style="style"
  >
    <slot>
      <span class="s-divider__text" v-if="description">{{ description }}</span>
    </slot>
  </div>
</template>
